                ...
                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center ${!isEven ? 'lg:flex-row-reverse' : ''}`}>
                  <div className={`relative overflow-hidden rounded-sm ${!isEven ? 'lg:order-2' : ''}`}>
                    <img
                      src={sol.img}
                      alt={sol.headline}
                      className={`w-full h-56 sm:h-72 lg:h-[400px] object-cover ${brightImage ? 'brightness-110 contrast-[1.03]' : ''}`}
                    />
                  </div>

                  <div className={!isEven ? 'lg:order-1' : ''}>
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-9 h-9 bg-teal/10 rounded-sm flex items-center justify-center">
