...
export default function Corporations() {
  ...
  const validate = () => {
    ...
    if (!form.org.trim()) errs.org = t('corporations.form_org_required');
    if (!form.reason) errs.reason = t('corporations.form_reason_required');
  ...
  return (
    <div className="overflow-hidden bg-white">
      <SEOHead
        title={t('seo.corporations.title')}
        description={t('seo.corporations.description')}
        pagePath="/solutions/corporations"
        keywords={t('seo.corporations.keywords').split(', ')}
      />
      ...
      <section className="bg-white">
        <div className="flex flex-col lg:flex-row lg:min-h-[520px]">
          <div className="w-full lg:w-1/2 px-8 md:px-12 lg:px-14 py-14 lg:py-20 flex flex-col justify-center">
            <SlotIn>
              <p className="text-teal text-xs font-heading font-semibold uppercase tracking-widest mb-3">{t('corporations.hero_label')}</p>
              <h1 className="font-heading text-3xl lg:text-4xl font-bold text-navy mb-8 leading-tight">
                {t('corporations.hero_title')}
              </h1>

              <div className="mb-6">
                <h2 className="font-heading font-bold text-navy text-lg mb-3">{t('corporations.hero_challenge')}</h2>
              ...
              <div className="mb-6">
                <h2 className="font-heading font-bold text-navy text-lg mb-3">{t('corporations.hero_solution')}</h2>
                <ul className="space-y-2">
              ...
              <div className="mb-8">
                <h2 className="font-heading font-bold text-navy text-lg mb-3">{t('corporations.hero_benefits')}</h2>
                <ul className="space-y-2">
                ...
                <div className="mt-6">
                  <Button asChild className="bg-navy hover:bg-navy/90 text-white font-semibold px-6 h-10 rounded-[6px] text-sm tracking-wide uppercase">
                    <Link to="/products">{t('corporations.hero_products_btn')}</Link>
                  </Button>
                </div>
              </div>

              <p className="font-heading font-semibold text-navy text-sm">
                {t('corporations.hero_partner')}
              </p>
            </SlotIn>
          </div>
      ...
      <section className="bg-surface py-14 lg:py-20 px-4">
        <div className="max-w-2xl mx-auto">
          <SlotIn>
            <h2 className="font-heading font-bold text-navy text-2xl lg:text-3xl mb-8 text-center">{t('corporations.form_heading')}</h2>
          </SlotIn>
          <SlotIn delay={0.1}>
              ...
              {!submitted ? (
                <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                  ...
                  <div className="flex justify-center pt-1">
                    <Button type="submit" disabled={loading} className="bg-navy hover:bg-navy/90 text-white font-semibold px-10 h-10 rounded-[6px] text-sm tracking-wide uppercase">
                      {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : t('corporations.form_submit')}
                    </Button>
                  </div>
                </form>
              ) : (
                <div className="py-12 text-center">
                  ...
                  <Button onClick={() => { setSubmitted(false); setForm(INITIAL_FORM); }} className="mt-8 bg-navy hover:bg-navy/90 text-white font-semibold px-8 rounded-[6px]">
                    {t('corporations.form_send_another')}
                  </Button>
                </div>
              )}
            </div>
