...
export default function Governments() {
  ...
  const validate = () => {
    ...
    if (!form.org.trim()) errs.org = t('governments.form_org_required');
    if (!form.reason) errs.reason = t('governments.form_reason_required');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/send-contact`, {
        ...
        body: JSON.stringify({ ...form, enquiryType: form.reason, profileType: 'government', token }),
      });
  ...
  const challengeBullets = t('governments.challenge_bullets', { returnObjects: true });
  const solutionBullets  = t('governments.solution_bullets',  { returnObjects: true });
  const benefits         = t('governments.benefits',          { returnObjects: true });
    ...
    <div className="overflow-hidden bg-white">
      ...
      <section className="bg-white">
        <div className="flex flex-col lg:flex-row lg:min-h-[520px]">
          ...
          <div className="w-full lg:w-1/2 px-8 md:px-12 lg:px-14 py-14 lg:py-20 flex flex-col justify-center">
            <SlotIn>
              <p className="text-teal text-xs font-heading font-semibold uppercase tracking-widest mb-3">{t('governments.hero_label')}</p>
              <h1 className="font-heading text-3xl lg:text-4xl font-bold text-navy mb-8 leading-tight">
                {t('governments.hero_title')}
              </h1>

              {/* Challenge */}
              <div className="mb-6">
                <h2 className="font-heading font-bold text-navy text-lg mb-3">{t('governments.hero_challenge')}</h2>
              ...
              {/* Othalo solution */}
              <div className="mb-6">
                <h2 className="font-heading font-bold text-navy text-lg mb-3">{t('governments.hero_solution')}</h2>
                <ul className="space-y-2">
              ...
              {/* Key benefits */}
              <div className="mb-8">
                <h2 className="font-heading font-bold text-navy text-lg mb-3">{t('governments.hero_benefits')}</h2>
                <ul className="space-y-2">
                ...
                <div className="mt-6">
                  <Button asChild className="bg-navy hover:bg-navy/90 text-white font-semibold px-6 h-10 rounded-[6px] text-sm tracking-wide uppercase">
                    <Link to="/products">{t('governments.hero_products_btn')}</Link>
                  </Button>
                </div>
              </div>

              <p className="font-heading font-semibold text-navy text-sm">
                {t('governments.hero_partner')}
              </p>
            </SlotIn>
          </div>
          {/* Right: image */}
          <div className="w-full lg:w-1/2 min-h-[300px] lg:min-h-0">
            <img
              src="/images/The District 2a.png"
              alt="Government housing solutions"
      ...
      <section className="bg-surface py-14 lg:py-20 px-4">
        <div className="max-w-2xl mx-auto">
          <SlotIn>
            <h2 className="font-heading font-bold text-navy text-2xl lg:text-3xl mb-8 text-center">{t('governments.form_heading')}</h2>
          </SlotIn>
          <SlotIn delay={0.1}>
              ...
              {!submitted ? (
                <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                  ...
                  <div className="flex justify-center pt-1">
                    <Button type="submit" disabled={loading} className="bg-navy hover:bg-navy/90 text-white font-semibold px-10 h-10 rounded-[6px] text-sm tracking-wide uppercase">
                      {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : t('governments.form_submit')}
                    </Button>
                  </div>
                </form>
              ) : (
                <div className="py-12 text-center">
                  ...
                  <Button onClick={() => { setSubmitted(false); setForm(INITIAL_FORM); }} className="mt-8 bg-navy hover:bg-navy/90 text-white font-semibold px-8 rounded-[6px]">
                    {t('governments.form_send_another')}
                  </Button>
                </div>
              )}
            </div>
